...
class StoryModeSelectSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		for (i in 0...CoolUtil.difficultyStuff.length) {
			menuItems.push(CoolUtil.difficultyStuff[i][0]);
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (accepted)
		{
			PlayState.storyPlaylist = ['Bozo', 'Blitzen', 'Roundabout'];
			PlayState.SONG = Song.loadFromJson(Highscore.formatSong('bozo', curSelected), 'bozo');
	...
	function changeSelection(change:Int = 0):Void
	{
		...
		if (curSelected >= menuItems.length)
			curSelected = 0;
		intendedScore = Highscore.getWeekScore('clown', curSelected);
